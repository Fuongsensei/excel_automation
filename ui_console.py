...
def  save_selected_keyins(data,path:str) -> list[int]:
        
        ctypes.windll.kernel32.SetFileAttributesW(yaml_path, 0)
        with open(path,mode='r') as file:
                config = yaml.safe_load(file)
        select =  input(f'Các số SAP keyins {apply_color(data['SAP'][config['selected_keyins']])} đang được chọn bạn có muốn chọn lại [{apply_color('Y')}/N]? : ').strip()
        if select.upper() == 'Y':
            ...
            config['selected_keyins'] = keyins_list
            print(config['selected_keyins'])
